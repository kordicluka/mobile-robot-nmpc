...
\textbf{Robot} & Polumjer & $r_{rob}$ & $0{,}2$ m \\
 ...
 & Maks.\ promjena brzine & $\Delta v_{max}$ & $0{,}5$ m/s\textsuperscript{2} \\
 & Maks.\ promjena kut.\ brzine & $\Delta\omega_{max}$ & $1{,}0$ rad/s\textsuperscript{2} \\
\midrule
\textbf{NMPC} & Predikcijski horizont & $N$ & 20 koraka \\
 & Korak uzorkovanja & $\Delta t$ & $0{,}1$ s \\
 & Ponderi pozicije & $Q_x, Q_y$ & $10{,}0$ \\
 & Ponder orijentacije & $Q_\theta$ & $1{,}0$ \\
 & Terminalna matrica troška & $W_e$ & $Q$ \\
 & Ponderi upravljanja & $R_{\Delta v}, R_{\Delta\omega}$ & $0{,}1$ \\
...
Slika~\ref{slk:baseline_traj} pokazuje da stvarna putanja gotovo u potpunosti prati referentnu. Jedino vidljivo odstupanje pojavljuje se na početku kretanja gdje robot s $\theta_0 = 0$ mora korigirati smjer prema dijagonalnom referentnom pravcu (kut $\approx 45°$). NMPC regulator ispravno prepoznaje taj zahtjev i robota glatko preusmjerava bez oscilacija.
...
\begin{tabular}{lll}
\toprule
Metrika & Vrijednost & Usporedba s baseline \\
\midrule
RMSE & $0{,}0469$ m & $+36{,}1$\% \\
...
RMSE i CTE su veći nego u baseline scenariju jer putanja sada ima zavoj oko prepreke --- robot ne može trenutačno promijeniti smjer pa u krivini blago zaostaje za referencom. Unatoč tome, RMSE ostaje ispod $50$ mm, a CPU overhead ($+8{,}8$\%) je zanemariv.
...
Ostvareni RMSE ($34{,}4$ mm) jednak je onom iz baseline scenarija, a greška smjera ($1{,}59°$) je najmanji od svih scenarija. Razlog je jednostavan: robot kreće s početnom orijentacijom $\theta_0 = 0$ (prema desno) i cilj se nalazi upravo u tom smjeru, pa ne treba inicijalna korekcija kuta --- za razliku od baseline scenarija gdje je robot morao odmah skrenuti za $\approx 45°$.
...
Najveći inicijalni vrh greške ($0{,}285$ m) nastaje kombinacijom greške pozicije i greške smjera na startu --- robot stoji na $y = 1{,}0$ m s $\theta_0 = 0$, dok referentna putanja odmah skreće prema $y \approx 2{,}2$ m. Bez tog tranzijenata prosječna greška u oba segmenta hodnika bila bi usporediva s jednostavnijim scenarijima.
...
Scenarij demonstrira neophodnost dvoslojne arhitekture: A* koji vidi cijeli prostor odjednom pronalazi jedini prohodani put --- zaobilazeći U-oblik izvana s desne strane. NMPC s horizontom $N = 20$ (predikcija $= 2$ s) pak ne može sam zaključiti o topologiji zamke, pa bi bez globalne reference zaglaviti unutar U.
...
Uspješno dovršavanje ovog scenarija potvrđuje ispravnost dvoslojne arhitekture. RMSE od $55{,}0$ mm drugi je po redu (iza L-hodnika) jer svaki od tri zavoja nakratko podigne grešku, no između zavoja praćenje je na razini baseline scenarija.

% ─────────────────────────────────────────────────────────────────────────────
\section{Poremećaj i oporavak}
\label{sec:r_perturbation}

Scenarij ispituje sposobnost oporavka sustava od nagle vanjske sile. Konfiguracija je identična baseline scenariju (ravna putanja od $(0{,}5;\; 5{,}0)$ do $(9{,}5;\; 5{,}0)$), s tom razlikom da se u koraku $k = 30$ ($t = 3{,}0$ s) stanje robota pomakne za $\Delta y = +1{,}0$ m. Poremećaj modelira nagli udarac --- događaj koji sustav ne može predvidjeti jer nije dio modela.
...
\begin{figure}[htbp]
  ...
  \caption{Upravljački profili u scenariju gustog rasporeda. Robot dosljedno održava brzinu oko $0{,}65$--$0{,}70$ m/s zbog profiliranja prema zakrivljenosti putanje.}
  \label{slk:cluttered_controls}
\end{figure}

Stvarna brzina dosljedno se zadržava ispod $v_{max} = 1{,}0$ m/s jer profil brzine izračunat prema zakrivljenosti ($v_{ref} = v_{max} / (1 + k\kappa)$) ispravno zaključuje da zakrivljena putanja između sedam prepreka ne dopušta vožnju punom brzinom.
...
RMSE od $42{,}9$ mm je iznimno dobar rezultat za scenarij s najvećim brojem prepreka --- bolji od L-hodnika ($72{,}4$ mm) i U-oblika ($55{,}0$ mm). Razlog je glatka dijagonalna putanja bez naglih zavoja od $90°$. CPU overhead od $17{,}6$\% u odnosu na baseline potvrđuje da dodavanje sedam mekih ograničenja nema značajan utjecaj na računalni teret.

% ─────────────────────────────────────────────────────────────────────────────
\section{Analiza osjetljivosti: predikcijski horizont $N$}
\label{sec:r_horizon}

Predikcijski horizont $N$ temeljni je projektni parametar koji definira broj koraka unaprijed u OCP-u. Duži horizont teorijski omogućuje bolju anticipaciju, ali povećava dimenzionalnost optimizacijskog problema i time računalni trošak. Analiza je provedena za $N \in \{5,\ 10,\ 15,\ 20,\ 30,\ 40\}$ na scenariju s jednom preprekom.
...
\begin{itemize}
  \item \textbf{Kratki horizont ($N = 5$):} RMSE iznosi $95{,}7$ mm. Kratka predikcija ograničava anticipaciju zakrivljene putanje izbjegavanja prepreke.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Analiza osjetljivosti: ponderi troška}
\label{sec:r_weights}
...
\begin{table}[H]
\centering
\caption{Utjecaj konfiguracije pondera na preciznost praćenja}
\label{tab:weights}
\begin{tabular}{lllll}
\toprule
...
Rezultati pokazuju monoton redosljed: preciznost praćenja raste s porastom omjera $Q/R$. Konfiguracija Q$\gg$R postiže najmanji RMSE ($41{,}5$ mm), no po cijenu agresivnijih upravljačkih akcija. Konfiguracija Q$<$R ($Q = 1{,}0$, $R = 5{,}0$) postiže daleko najlošiji RMSE ($272{,}9$ mm) jer optimizator ``bira'' manje upravljačkih akcija kako bi smanjio trošak $R$, što rezultira trajnom velikom greškom pozicije.
...
A* pretraživač iscrpljuje sve slobodne čvorove lijeve polovice i ispravno detektira da put ne postoji:
