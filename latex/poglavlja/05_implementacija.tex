...
\begin{enumerate}
  \item pronađe se najbliži sljedeći waypoint na putanji
...
for step in range(max_steps):
    # napreduj do najbližeg sljedećeg waypointa
...
Napredak po putanji određuje se usporedbom udaljenosti. Na svakom koraku algoritam izračunava $d_{curr} = \|\mathbf{p} - \mathbf{w}_{idx}\|$ i $d_{next} = \|\mathbf{p} - \mathbf{w}_{idx+1}\|$ te pomiče indeks naprijed sve dok vrijedi $d_{next} \leq d_{curr}$. Uvjet se provjerava u petlji pa robot može preskočiti više waypointova odjednom pri većim brzinama. Slika~\ref{slk:waypoint_tracking} ilustrira mehanizam: dok je robot blizu $w_2$, vrijedi $d_{next} \gg d_{curr}$ i indeks stoji; čim robot prijeđe polovicu puta prema $w_3$, sljedeći waypoint postaje bliži i indeks se pomiče. Na taj način referentna točka uvijek odgovara stvarnoj poziciji robota u prostoru.
...
\textbf{RMSE} (\textit{Root Mean Square Error}) mjeri prosječno odstupanje od referentne putanje:
...
\textbf{CTE} (\textit{Cross-Track Error}) mjeri okomitu udaljenost od segmenta putanje --- za razliku od RMSE-a uzima u obzir smjer putanje:
...
\textbf{Greška smjera} (\textit{heading error}) je apsolutna kutna razlika između orijentacije robota i smjera putanje, izračunata korištenjem atan2 za ispravno rukovanje kutnim prelaskom:

\begin{equation}
  e_{\theta,k} = \left| \text{atan2}(\sin(\theta_k - \theta_{ref,k}),\ \cos(\theta_k - \theta_{ref,k})) \right|
\end{equation}

Direktno oduzimanje kutova $\theta_k - \theta_{ref,k}$ nije ispravno jer rezultat može izaći iz intervala $(-\pi, \pi]$; atan2 formulation osigurava da greška uvijek ostaje unutar tog intervala.
...
Sustav je testiran na osam scenarija različite složenosti. Tablica~\ref{tab:scenariji} prikazuje njihove ključne karakteristike.
